...
def write_html(root, name, path=(Path(__file__).parents[2]) / Path("gametrees")):
    print(f"ABS: {path.absolute()}")
    html = Element("html")
    ...
    main.write(path.resolve() / f"{name}.html")
...
write_html(form_graph(states), "one_state")
"""
